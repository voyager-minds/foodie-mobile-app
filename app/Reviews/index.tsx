...
import { reviewsStyles as styles } from "./styles";
...
export default function ReviewsScreen() {
    ...
    const fetchReviews = useCallback(
        async (page = 1) => {
            ...
            try {
                const res = await restaurantService.getRestaurantReviews(restaurantId, page);
                ...
                const validReviews = reviewItems.filter((item: any) =>
                    item &&
                    typeof item === 'object' &&
                    item.id &&
                    item.ratings &&
                ...
                setReviewsTotalPages(reviewsData.totalPages || Math.max(1, Math.ceil(validReviews.length / 10)));
            } catch (err) {
    ...
    const addReply = async (reviewId: string) => {
        ...
        try {
            ...
            setReviews((prev: Review[]) =>
                prev.map((r) =>
                    r.id === reviewId
                        ? { ...r, comments: [...(r.comments || []), res.data.comment || res.data] }
                        : r
                )
            );
            ...
            <View style={styles.reviewCard}>
                <View style={styles.reviewHeader}>
                    ...
                    <View style={{ flex: 1, marginLeft: 10 }}>
                        <Text style={styles.reviewerName}>{item.authorSub || "Anonymous"}</Text>
                    ...
                    <View style={styles.reviewRating}>
                        <Text style={styles.ratingNumber}>
                            {item.ratings ? calculateOverallRating(item.ratings).toFixed(1) : "0.0"}
                        </Text>
                    </View>
                </View>
                        ...
                        <View style={styles.ratingStars}>
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Ionicons
                                    ...
                                    name="star"
                                    size={12}
                                    color={star <= (item.ratings?.service || 0) ? "#ffb74d" : "#ddd"}
                                />
                            ))}
                            <Text style={styles.ratingValue}>{item.ratings?.service || 0}</Text>
                        </View>
                    </View>
                    <View style={styles.ratingRow}>
                        ...
                        <View style={styles.ratingStars}>
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Ionicons
                                    ...
                                    name="star"
                                    size={12}
                                    color={star <= (item.ratings?.ambience || 0) ? "#ffb74d" : "#ddd"}
                                />
                            ))}
                            <Text style={styles.ratingValue}>{item.ratings?.ambience || 0}</Text>
                        </View>
                    </View>
                    <View style={styles.ratingRow}>
                        ...
                        <View style={styles.ratingStars}>
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Ionicons
                                    ...
                                    name="star"
                                    size={12}
                                    color={star <= (item.ratings?.value || 0) ? "#ffb74d" : "#ddd"}
                                />
                            ))}
                ...
                {openReplyIds[item.id] && (
                    <View style={styles.replySection}>
                        {item.comments && item.comments.length > 0 ? (
                                ...
                                {item.comments.map((c: Comment) => (
                                    <View key={c.id} style={styles.commentItem}>
                                        <View style={styles.commentHeader}>
                                            <Text style={styles.commentAuthor}>{c.authorSub || "U"}</Text>
                            ...
                            <Text style={{ color: "#666", marginBottom: 8 }}>No replies yet</Text>
                        )}

                        <TextInput
                            placeholder="Write a reply..."
                            value={replyTexts[item.id] || ""}
                            onChangeText={(t: string) => setReplyTexts((p) => ({ ...p, [item.id]: t }))}
    ...
    if (!restaurantId) {
        return (
            <View style={styles.centered}>
                ...
                <TouchableOpacity
                    style={{ marginTop: 20, padding: 10, backgroundColor: '#ff7f50', borderRadius: 8 }}
                    onPress={() => router.back()}
                >
            ...
            <View style={styles.screen}>
                        ...
                        <View style={styles.ratingOverview}>
                            ...
                            <View style={styles.starsContainer}>
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <Ionicons
                                        ...
                                        name="star"
                                        size={16}
                                        color={star <= (restaurant.avgOverall || 0) ? "#ffb74d" : "#ddd"}
                                    />
                                ))}
                            </View>
                            ...
                            <View style={styles.avgRatingsBreakdown}>
                                <View style={styles.avgRatingRow}>
                                    ...
                                    <Text style={styles.avgRatingValue}>{restaurant.avgFood?.toFixed(1) || "0.0"}</Text>
                                </View>
                                <View style={styles.avgRatingRow}>
                                    <Text style={styles.avgRatingLabel}>Service:</Text>
                                    <Text style={styles.avgRatingValue}>{restaurant.avgService?.toFixed(1) || "0.0"}</Text>
                                </View>
                                <View style={styles.avgRatingRow}>
                                    <Text style={styles.avgRatingLabel}>Ambience:</Text>
                                    <Text style={styles.avgRatingValue}>{restaurant.avgAmbience?.toFixed(1) || "0.0"}</Text>
                                </View>
                                <View style={styles.avgRatingRow}>
                                    <Text style={styles.avgRatingLabel}>Value:</Text>
                                    <Text style={styles.avgRatingValue}>{restaurant.avgValue?.toFixed(1) || "0.0"}</Text>
                                </View>
                            </View>
                        </View>
                        <TouchableOpacity
                ...
                {loadingReviews ? (
                    ...
                    <>
                        ...
                        <FlatList
                            data={reviews}
                        keyExtractor={(r: Review) => r.id?.toString() || Math.random().toString()}
                        ...
                        ListFooterComponent={
                            reviewsTotalPages > 1 ? (
                                <View style={styles.pagination}>
                                    <TouchableOpacity
                                        style={[styles.pageButton, reviewsPage === 1 && styles.disabledBtn]}
                                ...
                                <View style={styles.ratingInput}>
                                    ...
                                    <View style={styles.starRating}>
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <TouchableOpacity
                                                key={star}
                                                onPress={() => setNewReviewRatings(prev => ({ ...prev, food: star }))}
                                            >
                                                <Ionicons
                                                    name="star"
                                                    size={20}
                                                    color={star <= newReviewRatings.food ? "#ffb74d" : "#ddd"}
                                                />
                                            </TouchableOpacity>
                                        ))}
                                        <Text style={styles.modalRatingNumber}>{newReviewRatings.food}</Text>
                                    </View>
                                </View>
                                ...
                                <View style={styles.ratingInput}>
                                    ...
                                    <View style={styles.starRating}>
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <TouchableOpacity
                                                key={star}
                                                onPress={() => setNewReviewRatings(prev => ({ ...prev, service: star }))}
                                            >
                                                <Ionicons
                                                    name="star"
                                                    size={20}
                                                    color={star <= newReviewRatings.service ? "#ffb74d" : "#ddd"}
                                                />
                                            </TouchableOpacity>
                                        ))}
                                        <Text style={styles.modalRatingNumber}>{newReviewRatings.service}</Text>
                                    </View>
                                </View>
                                ...
                                <View style={styles.ratingInput}>
                                    ...
                                    <View style={styles.starRating}>
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <TouchableOpacity
                                                key={star}
                                                onPress={() => setNewReviewRatings(prev => ({ ...prev, ambience: star }))}
                                            >
                                                <Ionicons
                                                    name="star"
                                                    size={20}
                                                    color={star <= newReviewRatings.ambience ? "#ffb74d" : "#ddd"}
                                                />
                                            </TouchableOpacity>
                                        ))}
                                        <Text style={styles.modalRatingNumber}>{newReviewRatings.ambience}</Text>
                                    </View>
                                </View>
                                ...
                                <View style={styles.ratingInput}>
                                    ...
                                    <View style={styles.starRating}>
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <TouchableOpacity
                                                key={star}
                                                onPress={() => setNewReviewRatings(prev => ({ ...prev, value: star }))}
                                            >
                                                <Ionicons
                                                    name="star"
                                                    size={20}
                                                    color={star <= newReviewRatings.value ? "#ffb74d" : "#ddd"}
                                                />
                                            </TouchableOpacity>
                                        ))}
                                        <Text style={styles.modalRatingNumber}>{newReviewRatings.value}</Text>
                                    </View>
                                </View>
                            </View>

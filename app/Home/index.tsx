...
export default function Index() {
    ...
    if (filters.city || filters.category) {
      filtered = filtered.filter(
        (r) =>
          ...
          (!filters.category ||
            (r.category && r.category.toLowerCase().includes(filters.category.toLowerCase())) ||
  ...
  const renderRestaurant = ({ item }: { item: Restaurant }) => (
    <View style={styles.card}>
      <Image source={{ uri: getImageUrl(item.coverImageKey) }} style={styles.image} />
        ...
        <View style={styles.filtersCard}>
          <View style={styles.filtersHeader}>
            <View style={styles.filterIconContainer}>
              <Ionicons name="options" size={20} color="#ff7f50" />
            </View>
            <Text style={styles.filtersTitle}>Find Your Perfect Restaurant</Text>
          </View>
          ...
          <View style={styles.filtersContent}>
            ...
            <View style={styles.inputContainer}>
              <View style={styles.inputIconContainer}>
                <Ionicons name="location-outline" size={18} color="#ff7f50" />
              </View>
              <TextInput
            ...
            <Text style={styles.quickFiltersLabel}>Popular cuisines:</Text>
            <View style={styles.quickFiltersRow}>
              {['Sri Lankan', 'Chinese', 'Indian', 'Italian', 'Thai'].map((cuisine) => (
                <TouchableOpacity
                  key={cuisine}
                  style={[
                    styles.quickFilterTag,
                    filters.category.toLowerCase() === cuisine.toLowerCase() && styles.quickFilterTagActive
                  ]}
                  onPress={() => {
                    Keyboard.dismiss();
                    setFilters({ ...filters, category: cuisine });
                  }}
                >
                  <Text style={[
                    styles.quickFilterText,
                    filters.category.toLowerCase() === cuisine.toLowerCase() && styles.quickFilterTextActive
                  ]}>
                    {cuisine}
                  </Text>
                </TouchableOpacity>
            ...
            <View style={styles.filterButtons}>
              <TouchableOpacity
                style={[styles.applyButton, (filters.city || filters.category) && styles.applyButtonActive]}
                onPress={onFilter}
              ...
              {(filters.city || filters.category) && (
                <TouchableOpacity style={styles.clearButton} onPress={clearFilters}>
                  <Ionicons name="refresh" size={14} color="#ff7f50" />
            ...
            <View style={styles.activeFilters}>
              {searchTerm.trim() !== "" && (
                <View style={styles.activeFilterTag}>
                  <Ionicons name="search" size={12} color="#ff7f50" />
                  <Text style={styles.activeFilterText}>"{searchTerm}"</Text>
                </View>
              )}
              {filters.city && (
                <View style={styles.activeFilterTag}>
                  <Ionicons name="location" size={12} color="#ff7f50" />
                  <Text style={styles.activeFilterText}>{filters.city}</Text>
                </View>
              )}
              {filters.category && (
                <View style={styles.activeFilterTag}>
                  <Ionicons name="restaurant" size={12} color="#ff7f50" />
